...
[[ "" != "$appdir" ]] || { echo 使用默认项目存储路径 /var/www/project && appdir="/var/www/project";}
echo "项目存储路径为:$appdir"
baseurl=`{ ping -c 1 -W 1 customproject.oss-cn-shanghai-internal.aliyuncs.com &>/dev/null && echo customproject.oss-cn-shanghai-internal.aliyuncs.com ||
echo customproject.oss-cn-shanghai.aliyuncs.com; }`
