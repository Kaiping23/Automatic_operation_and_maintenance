...
#
echo 检测merge_from分支
if git checkout $merge_from
then git pull
else echo $merge_from 不存在，自动从develop创建 && git checkout develop &&git pull&&git checkout  -b $merge_from
fi
...
then echo 无merge操作，跳过
     ...
     echo 检测merge_to分支
    if git checkout $merge_to
    then git pull
    else echo $merge_to 不存在，自动从develop创建 && git checkout develop &&git pull&&git checkout  -b $merge_to
    fi
     git merge --commit --ff $merge_from || { echo merge 失败退出; exit 1; }
...
git push --tags --set-upstream origin $merge_to || { echo push 失败退出; exit 1; }
echo
echo
...
then echo "java project $project,开始打包"
    group=wmy-backend
    if [[ "$job" != "" ]]
    then echo "*********指定了混合项目的真实目录，进入真实目录执行***********"
	...
	echo "real_dir ---> $real_dir"
		...
		echo " project_dir --> $project_dir"
		echo "cd $BASEDIR && cp -r $project_dir cloud-$project_dir-service && mv cloud-$project_dir-service $project_dir && cd $project_dir";
    ...
    if [[ $? = 0 ]]
    then echo 打包完成
    else echo 打包失败，删除缓存仓库，重新打包api和service，可能会影响其他正在打包的程序
    ...
    else target_dir="`ls -d */ |grep  $project |xargs -n 1|grep -v api`/target"
    fi
    # 检查打包目录是否存在
    if [ -d "$target_dir" ]
    ...
    ln  $BASEDIR/$project_dir/$target_dir/*jar $docker_dir/jar/ || cp $BASEDIR/$project_dir/$target_dir/*jar $docker_dir/jar/
     ...
     if [[ "$project" = "admin" ]] || [[ "$project" = "sassadmin" ]];
...
if [ "$project" = "cleverbee" ];
	then echo "---cleverbee项目特殊处理 cd $BASEDIR && rm -rf $project_dir " && cd $BASEDIR && rm -rf $project_dir;
fi
wait
}
