...
docker_repo="192.168.0.140:9000" #镜像地址不包含命名空间及仓库地址
...
then git pull
else echo $merge_from 不存在，自动从master创建 && git checkout master &&git pull&&git checkout  -b $merge_from
fi
...
then echo 无merge操作，跳过
    ...
    then git pull
    else echo $merge_to 不存在，自动从master创建 && git checkout master &&git pull&&git checkout  -b $merge_to
    fi
...
then echo "java project $project,开始打包"
    ...
    then echo "*********指定了混合项目的真实目录，进入真实目录执行***********"
        ...
        if [[ ! -d "$real_dir" ]];then echo 包含${project}的真实的项目目录不存在，请检查;exit 1;fi
        cd $real_dir
