...
echo "project-name："$appname && echo "JAVA_OPTS: "${JAVA_OPTS}
 echo "执行命令如下："java ${JAVA_OPTS} $javaopts -Djava.security.egd=file:/dev/./urandom -jar /jar/*jar $argopts
java ${JAVA_OPTS} $javaopts -Djava.security.egd=file:/dev/./urandom -jar /jar/*jar $argopts
